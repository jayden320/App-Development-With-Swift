//: ## 起名字
//: 在编程的时候，名字是非常有用的，名字可以帮助记住阿猫是阿猫，阿狗是阿狗 - 就像你给现实生活中的东西起名字一样。在代码中，一旦你给一个值起了一个名字，你就可以在需要使用该值的地方，使用该名称。这样可以使代码更容易阅读和理解。
//:
//: 名字还可以帮助你避免很多错误。例如，在宠物展会的练习中很容易出错，因为每一次数量的变动，你都必须在多行代码中更改同一个数字。但是，如果你给每个值起了一个名字（例如，乌龟的数量），你只需要更改一次名称的定义，代码中出现这个名字的地方，就会自动更新为新的值。
//:
...
//: 根据下面的信息，更新代码:
//: - 另外一个人带来了两只狗。
//: - 生病的乌龟感觉好多了，重新来参加宠物展会。
//: - 有一只猫不能来了。
//: - 另一个人带着一只老鼠。
//:
//: 一旦你完成练习，就继续前进吧。
//:
//:[上一页](@previous)  |  第4页，共14页  |  [下一页：代码补全](@next)
